...
class ANETdetection(object):
    ...
    def _import_ground_truth(self, annotation_path):
        ...
        gtsegments = np.delete(gtsegments, [402, 226])

        gtlabels = np.load(annotation_path + "/labels.npy", allow_pickle=True)
        gtlabels = np.delete(gtlabels, [402, 226])

        videoname = np.load(annotation_path + "/videoname.npy", allow_pickle=True)
        videoname = np.delete(videoname, np.where(videoname == b'video_test_0001496'))  # wrong annotation
        videoname = np.delete(videoname, np.where(videoname == b'video_test_0001292'))
        videoname = np.delete(videoname, np.where(videoname == b'video_test_0000270'))
        ...
        for i in range(len(gtsegments)):
            ...
            for j in range(len(gtsegments[i])):
                ...
                t_start_lst.append(float(gtsegments[i][j][0] * t_factor))
                t_end_lst.append(float(gtsegments[i][j][1] * t_factor))
